...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const onTeam = await db.get('SELECT p.*, r.roleid, t.logo, t.playercount FROM Players p, Roles r, Teams t WHERE r.code = p.team AND p.team = t.code AND p.discordid = ?', userid);
        console.log(onTeam)
        if (!onTeam) {
            await db.close()
        ...
        await db.run('UPDATE Players SET demands = demands - 1, team = "FA", role = "P", contractlength = -1  WHERE discordid = ?', userid);
        ...
        await db.run('UPDATE Teams SET playercount = playercount - 1 WHERE code = ?', onTeam.team);
        ...
        const roleId = await db.get('SELECT roleid FROM Roles WHERE code = ?', onTeam.team)
        await interaction.member.roles.remove(roleId.roleid)
        if (onTeam.role === "HC" || onTeam.role === "GM") {
            const specialRoleId = await db.get('SELECT roleid FROM Roles WHERE code = ?', onTeam.role)
            await interaction.member.roles.remove(specialRoleId.roleid)
        }

        // then, dm the franchise owner notifying them
        const foID = await db.get("SELECT discordid FROM Players WHERE team = ? AND role = 'FO'", onTeam.team)
