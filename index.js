...
client.on(Events.GuildDelete, async guild => {
	try {
		const db = await getDBConnection()
		const guildid = guild.id
		await db.run('DELETE FROM Players WHERE guild = ?', guildid)
		await db.run('DELETE FROM Teams WHERE guild = ?', guildid)
		await db.run('DELETE FROM Leagues WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM Roles WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM Admins WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM Managers WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM Results WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM Channels WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM Suspensions WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM QBStats WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM WRStats WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM RBStats WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM DefenseStats WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM KStats WHERE guild = ?', guildid) // no need to check if channel exists
		await db.run('DELETE FROM Trades WHERE guild = ?', guildid) // no need to check if channel exists
		await db.close()
	} catch(err) {
		console.log(err)
	}
